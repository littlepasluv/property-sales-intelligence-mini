...
from app.services.analytics_service import is_sla_breached
...
def calculate_confidence_score(lead: Lead, analytics_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Calculates a confidence score for a single lead based on data completeness and signal consistency.
    """
    score = 1.0
    
    # 1. Data Completeness Penalty
    if not lead.email: score -= 0.1
    if not lead.budget: score -= 0.1
    if not lead.notes or len(lead.notes) < 10: score -= 0.05
...
def calculate_explainability_coverage(risk_factors: List[Dict[str, Any]]) -> float:
    """
    Calculates the percentage of expected risk factors that were identified for a lead.
    """
    if not risk_factors:
        return 0.0
    
    identified_factor_types = {factor['type'] for factor in risk_factors}
...
def calculate_data_freshness(leads: List[Lead]) -> Dict[str, Any]:
    ...
    """
    if not leads:
        return {"last_updated_at": None, "freshness_score": 0}

    latest_timestamp = max(
        [lead.created_at for lead in leads] +
        [f.created_at for lead in leads for f in lead.followups if f.created_at]
    )
    
    # Ensure latest_timestamp is offset-aware for comparison
